from tkinter import * #impordib tkinter moodulist

def save_info(): #defineerib info salvestamist
    firstname_info = firstname.get() #eesnime info
    lastname_info = lastname.get() # perekonnanime info
    age_info = age.get() #vanuse info
    age_info = str(age_info) #vanuse info string
    print(firstname_info, lastname_info, age_info) #prindib vanuse,ees- ja perekonnanime info

    file = open("user.txt", "w") #
    file.write(firstname_info) #kirjutab faili eesnime info
    file.write(lastname_info) #kirjutab faili perekonnanime info
    file.write(age_info) #kirjutab faili vanuse info
    file.close() #sulgeb faili
    print(" User ", firstname_info, " has been registered successfully") #prindib kasutaja eesnime ja annab teada et see on registreeritud
...
sreen.mainloop()  # Käivitame ekraani